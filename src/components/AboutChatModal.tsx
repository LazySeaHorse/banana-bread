  ...
  return (
    <div className="h-2 flex-1 overflow-hidden rounded-full bg-neutral-100">
      <div
        ...
        style={{ width: `${Math.max(pct, 2)}%`, background: color ?? "linear-gradient(90deg,#5B51D8,#E1306C)" }}
      />
    </div>
  );
}

export function AboutChatModal({ chat, onClose }: { chat: ChatData; onClose: () => void }) {
      ...
      <div className="flex h-full w-full max-w-md flex-col overflow-y-auto bg-white shadow-2xl md:h-[88vh] md:rounded-2xl">
        ...
        <div className="flex flex-col items-center gap-2 border-b border-neutral-100 px-4 py-6">
          ...
          <div className="text-xs text-neutral-400">
            {formatFullDate(stats.firstTs)} — {formatFullDate(stats.lastTs)} · {stats.durationDays} days
          </div>
        </div>

        {/* who am i */}
        <section className="border-b border-neutral-100 px-4 py-4">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-neutral-400">Who are you?</h3>
          <div className="flex flex-col gap-1">
            {chat.participants.map((p) => (
              ...
              >
                ...
                <span className="flex-1 text-sm font-medium text-neutral-800">{p}</span>
                {chat.me === p && (
        ...
        {/* theme */}
        <section className="border-b border-neutral-100 px-4 py-4">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-neutral-400">Bubble color</h3>
            ...
            {BUBBLE_THEMES.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  chat.theme.id === t.id ? "ring-neutral-800" : "ring-transparent"
                )}
                title={t.label}
                style={{ background: `linear-gradient(135deg, ${t.meFrom}, ${t.meTo})` }}
              >
                {chat.theme.id === t.id && <Check size={16} className="text-white" />}
              </button>
            ))}
          </div>
          <CustomGradientPicker chatId={chat.id} theme={chat.theme} onChange={(t) => setTheme(chat.id, t)} />
        </section>
              ...
              <div key={p.name} className="flex items-center gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-baseline justify-between text-xs">
                    <span className="truncate font-medium text-neutral-700">{p.name}</span>
          ...
          <div className="flex h-16 items-end gap-0.5">
            {stats.hourHistogram.map((v, h) => (
              <div
                ...
                style={{ height: `${(v / maxHour) * 100}%`, minHeight: v > 0 ? 3 : 1, opacity: v > 0 ? 1 : 0.15 }}
              />
            ))}
          </div>
            ...
            {stats.weekdayHistogram.map((v, d) => (
              <div key={d} className="flex flex-1 flex-col items-center gap-1">
                <div
                  ...
                  style={{ height: `${(v / maxWeekday) * 48}px`, minHeight: v > 0 ? 3 : 1, opacity: v > 0 ? 1 : 0.15 }}
                />
                <span className="text-[9px] text-neutral-400">{WEEKDAY_LABELS[d]}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        {stats.topEmojis.length > 0 && (
          <section className="border-b border-neutral-100 px-4 py-4">
            <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-wide text-neutral-400">Top emoji</h3>
            <div className="flex flex-wrap gap-2">
        ...
        <section className="border-b border-neutral-100 px-4 py-4 text-sm text-neutral-600">
          <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-neutral-400">Fun facts</h3>
          {stats.mostActiveDay && (
            <p className="mb-1">
              🔥 Most active day: <b>{new Date(stats.mostActiveDay.date).toLocaleDateString()}</b> with{" "}
              {stats.mostActiveDay.count} messages
            </p>
          )}
          {stats.longest && (
            <p>
              ✍️ Longest message by <b>{stats.longest.sender}</b> ({stats.longest.length} chars)
            </p>
          )}
        </section>
    ...
    <div className="mt-3 flex items-center gap-3 rounded-xl bg-neutral-50 p-2.5">
      <div className="flex items-center gap-1.5">
        <input
          type="color"
          value={theme.meFrom}
          onChange={(e) => onChange({ ...theme, id: "custom", label: "Custom", meFrom: e.target.value })}
          className="h-7 w-7 cursor-pointer rounded-full border-0 bg-transparent p-0"
        />
        <input
          type="color"
          value={theme.meTo}
          onChange={(e) => onChange({ ...theme, id: "custom", label: "Custom", meTo: e.target.value })}
          className="h-7 w-7 cursor-pointer rounded-full border-0 bg-transparent p-0"
        />
      </div>
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${theme.meFrom}, ${theme.meTo})` }}
      />
    </div>
  );
}
