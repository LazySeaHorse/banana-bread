...
export const PROVIDER_MODEL_OPTIONS: Record<AIProviderId, string[]> = {
  ...
  cohere: ["command-r-plus", "command-r", "command-a-plus-05-2026", "command-light"],
  groq: [
...
function sanitizeProviderConfig(provider: AIProviderId, raw: unknown): ProviderConfig {
  const fallback = emptyProviderConfig(provider);
  if (!raw || typeof raw !== "object") return fallback;
  const obj = raw as Record<string, unknown>;
  return {
    apiKey: typeof obj.apiKey === "string" ? obj.apiKey : fallback.apiKey,
    model: typeof obj.model === "string" && obj.model.trim() ? obj.model : fallback.model,
  };
}
...
function sanitizeSettings(raw: unknown): AISettings {
  ...
  const activeProvider = isProviderId(obj.activeProvider) ? obj.activeProvider : defaults.activeProvider;

  const providers = {} as Record<AIProviderId, ProviderConfig>;
  const rawProviders = (obj.providers && typeof obj.providers === "object" ? obj.providers : {}) as Record<
    string,
    unknown
  >;
  ...
  if (typeof obj.maxContextTokens === "number" && Number.isFinite(obj.maxContextTokens)) {
...
export function getActiveProviderConfig(settings: AISettings): ProviderConfig {
  return settings.providers[settings.activeProvider] ?? emptyProviderConfig(settings.activeProvider);
}

export function isProviderConfigured(settings: AISettings, provider: AIProviderId): boolean {
